...
## Caveats
* (thanks to Ari Laor) The opacity tables computed here are for the Semenov 2003 model, which is very much targeted at modeling protoplanetary disks, and chose a dust composition appropriate for those conditions following Pollack 1994. Notably, the model does not contain graphite grains, and thus it is not expected to extrapolate well to the diffuse ISM at the shorter wavelengths where those grains dominate the opacity - see [Baskin & Laor 2018 Fig. 6](https://ui.adsabs.harvard.edu/abs/2018MNRAS.474.1970B). 
